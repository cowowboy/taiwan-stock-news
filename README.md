...
## 公司行動前瞻行事曆（2026-09-03 起）

`corp_actions.json` — 未來的除權息、股票分割（面額變更）與減資。
目的是**提早部署**：分割/減資會有一段舊股停止交易期，那幾天部位是凍結的。

| | 來源（全部免金鑰） |
|---|---|
| 除權息預告 | TWSE `rwd/zh/exRight/TWT48U`、TPEx `openapi/v1/tpex_exright_prepost` |
| 分割／減資 | TWSE `openapi/v1/opendata/t187ap04_L`、TPEx `openapi/v1/mopsfin_t187ap04_O` |

**為什麼不用 FinMind**（2026-09-03 實測）：`TaiwanStockDividendResult`、
`TaiwanStockSplitPrice`、`TaiwanStockCapitalReductionReferencePrice` 查未來區間都是 **0 筆**，
它們是「生效當天才有」的結果表；`TaiwanStockDividend` 有未來除息日但不帶 `data_id` 只回 1 檔。
以寶雅 5904 為例，FinMind 的 `720 → 72` 出現在生效日 2026-08-10，**提前量 0 天**；
而重大訊息 2026-07-02 就公告了，**提前 39 天**。

`build_corporate_actions.py` 是**累積式**的：重大訊息是「每日」feed，
每次跑都跟既有檔案合併去重。公告會在來源留三個月，所以漏跑幾班補得回來。
排程 `corp-actions.yml` 每天兩班（07:20 / 21:20 台北，**含週末**——公告不挑交易日發）。

### 解析上踩過的坑（都在 `tests/test_corp_actions.py` 釘住）

五份真實公告當測試樣本，因為每一種寫法漏掉都是**靜默失敗**（行事曆少一檔不會報錯）：

| 樣本 | 難在哪 |
|---|---|
| 5314 世紀（上櫃） | 標題只有「本公司股票面額變更相關事宜」——沒有數字沒有期間，精確樣式會整檔漏掉；同一件事重貼 19 則 |
| 5904 寶雅（上櫃） | 阿拉伯序號 `(1)(2)`，標題含面額與公告期間 |
| 6919 康霈（上市） | 欄位編號 `4.換發股票時程(1)…`，日期帶「民國」前綴 |
| 6949 沛爾（上市） | 持續公告**不含時程表**，最後交易日藏在散文括號，且標籤被硬換行切開 |
| 5301 寶得利（上櫃） | 減資，標籤全是變體；計畫**尚未經主管機關核備**（`provisional: true`） |

`halt_source` 分「公告」與「推算」：公告沒寫停止交易期間時，用舊股最後交易日與
新股上市日推算——那兩天之間該公司無股可交易，是定義使然，不是猜。
寶得利同時寫了兩者，獨立算出來一致，這條當作推算邏輯的驗證釘在測試裡。

**已知限制**：強生 4747 那種持續公告寫「其他應敘明事項：無」，時程在另一則
一次性公告裡且已掉出 feed，只拿得到新股上市日與面額。缺的欄位一律留 `None`
讓前端顯示「公告未載」，**絕不用鄰近事件的值去補**。累積式抓取從開跑那天起才會完整。
...
| 角色 | 負責 |
|---|---|
| 排程 cloud session（台北一~五 08:15） | 讀資料、網路搜尋、判讀與組稿 → 產出 content JSON |
...
**排程時間的由來（2026-09-21 調整，07:52 → 08:15）**：晨報要讀 v2 的 `us.json`，
而它的 FinMind 來源約台北 08:00 才入庫。實測近兩週 `us.json` 帶到正確資料日的時間
多在 07:41，但 2026-09-14 那天晚到 **08:06**——而當期晨報 08:03 就提交了，
**用到的是前一個交易日的美股數字**。08:15 啟動、約 08:30 完成，離開盤 09:00 仍有 30 分鐘。

光改時間不夠：排程 prompt 另外要求**先檢查 `us.json` 的資料日**是不是
「今天的前一個美股交易日」，不是就不用它的隔夜數字、在該區塊註明「尚未到位」。
時間只降低機率，檢查才防止出錯。
...
| 把美股族群的單日數字寫成趨勢（9/17 判「熄火」，9/18 就翻正） | 數字欄位照抄上游、「昨夜」那格由程式排，模型不經手 |
| 改寫族群名把判讀塞進 `g` 繞過分欄設計 | `g` 必須是 `SECTOR_NAMES` 之一 |
| 把判讀寫成數字塞進 `view` | `view` 不准夾帶數值；中文數字要接單位才擋（「跌三趴」擋、「半導體仍偏弱」放行） |
| 上游族群檔停更時安靜地拿舊資料當昨夜 | `us_sectors` 非空就必須附 `us_sectors_date`，距今 >5 天直接擋，並渲染進區塊標題 |

`positioning.fact` 用**字元數**計——「46,164.72」的漢字數是 0，
套漢字下限會把最正常的一列誤殺。

`us_sectors` 走的是另一條路：**不驗文字，直接不要文字**。
`chg`／`pos`／`vr`／`t20` 是照抄上游的數字，`g` 必須是固定名單之一，
版面上「昨夜」那一格由程式排——模型只寫 `view` 一句判讀（且不准有數字）。
這條規則改過四版：禁字表、漢字上限、白名單三版都被驗收繞過，
原因一樣——寫的人永遠比過濾器有更多表達空間。第四版把自由文字本身拿掉。

`tests/test_brief_tools.py` 把上面每一條都釘成 CI 會擋的失敗，
並額外驗「所有下限相加（1,400 字）遠低於總上限」——避免上下限互相鎖死到無解。
族群區把真正的最壞情況（6 列 × 最長族群名 × `view` 寫滿 24 字元＝224 漢字）
釘成測試，並拿 **repo 裡真的那一期**當基準斷言總預算（目前 4,688＋224，
餘裕 88 漢字）——不是拿測試用的合成內容，那會虛報 25 倍餘裕。
這個區塊必須有**硬**上界，否則一個可有可無的裝飾區塊有能力讓整份晨報 exit 1 不寫檔。

`view` 的守備範圍測試也分成兩半、兩邊都有斷言：**帶數字的一律擋**
（含中文數字「跌三趴」、圈號、分數、上下標），**純語氣的一律放行**
（「動能熄火」「momentum fading fast」）。後者是刻意的——view 就是判讀欄，
驗證分不出「熄火」講的是昨夜還是多週；擋它的是旁邊那幾欄程式排的數字，
不是驗證。把分界釘成測試，免得日後有人誤以為這裡漏擋了。
