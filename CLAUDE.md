...
1. **「今日一句話」排版鐵則（2026-08-29 使用者回饋）**：禁止單一長段。依內容分成
   2–6 個短段，每段一個 `<p class="final">`、每段 ≤150 字、段首以粗體【小標】開頭
   （寫法 `<b>【小標】</b>`，例：【盤勢】【操作】【匯率】【工作】【家庭】【提醒】，
   依當日內容取捨）；以精煉收束為目標，不逐字重複前文區塊已寫過的數據細節。
2. **`daily-brief-card.json` 的 `quote` 欄**＝今日一句話的**濃縮版**：≤120 字、
