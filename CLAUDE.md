...
1. **速讀版硬預算（2026-08-30 第 24 期起，使用者裁定，取代 08-29 前的長文版式）**：
   當期正文（歷史存檔前）**≤5,000 漢字**；每則新聞＝h3 標題＋一句 why(16~60字)＋
   來源 meta，細節收 `<details class="more">`；重點判讀全刊限 3 則（各 50~150 字）；
   行情數字只在 `table.pos` 完整出現一次；`<b>` 每則至多 1 處；生活三~四區塊收合於
...
5. **字數是上下限，兩端都會被 `brief_tools.py` 擋（2026-09-03 第 28 期起）**：
   第一版只設上限，結果第 28 期正文只有 3,464 字（上游同日 5,064），
   1,545 字的落差幾乎全在生活區塊——模型看到的全是上限，就貼著上限以下寫。
   現在 `life.note` 200~500 字（**完整段落，不是一句話**）、`top3.why` 40~90 字、
   `calls` 三欄各 50~150 字、`quote` 40~100 字，寫太薄與寫太長一樣 exit 1。
6. **「今日三件事」每則必須有判讀層與來源（2026-09-03 起）**：
   `top3` 的 `why` 是【判讀】——說明這件事對今天台股的意義，不是把標題換句話說；
   `source` / `source_url`（須 `https://`）必填，版式會渲染成可點的來源行。
   **三件事至少一件是隔夜外部盤面**（美股／費半／ADR／匯率／油價／公債）——
   早晨晨報最該回答的是「台股睡覺時發生了什麼」。
   注意 `us.json` 是純市場數據、**一個網址都沒有**，所以這類事件要用 WebSearch
   找原始出處（官方新聞稿或主流媒體報導）、用 WebFetch 打開確認內容相符後才填
   `source_url`；不可自行拼湊網址。
   （第 29 期的教訓：`source_url` 一設成必填，session 就只挑得出 `news.json` 裡
   有 `link` 的台灣新聞，三件事全是台股內部消息，博通財報整個消失——
   而第 28 期反而寫得出來，因為當時沒有這條要求。補來源層時差點把隔夜美股那層擠掉。）
7. **「本週關鍵事件」必須真的在本週**：非空時至少 4 則的日期落在本週（週一~週日），
   程式會自己解析 `when` 裡的 M/D 驗證。月中的 FOMC、結算日可列為額外項目但不能充數。
   另：不得寫「無 X」這種非事件，也不得在任何欄位提到 json 檔名或「欄為空」
   ——讀者看不到那些檔案（第 28 期實際寫了「台股今日無除權息個股（morning.json exdiv 欄為空）」）。
8. **「今日關注個股」要接得上當天的驅動**：第 28 期只機械列了三檔處置股，
   與當天三件事無關。優先挑與三件事驅動相關的標的，處置股是沒有更好標的時的備選。
9. **「開盤前定位」一列一個市場，6~10 列（2026-09-03 第 30 期起）**：
   第 30 期只有 6 列，其中兩列各塞了四個市場（「美股指數」「三大法人與匯率」），
   上游同日是 12 列各自獨立——差的 216 漢字幾乎全在這裡。
   `positioning.fact` 的長度用**字元數**算 4~60（不是漢字數：「46,164.72」是 0 漢字，
   套漢字下限會把最正常的一列誤殺）；上限 60 字元就是用來擋「一列塞四個市場」。
   數字照寫，不要寫成句子——解讀寫在 `view`。

10. **「昨夜美股族群」的數字照抄，字串由程式組（2026-09-21 起）**：
    資料源是 taiwan-flow-live-v2 的 `data/us_sector_flow.json`（由該站
    `src/build_us_sectors.py` 在 `us.yml` 跑完 `us.json` 之後產出，**失敗不擋 us.json**）。
    整區可缺席——資料日不夠新就給空陣列，版面會連標題帶目錄一起不出現。
    - `g` 必須是 `brief_tools.py` 的 `SECTOR_NAMES` 之一（上游那 13 個族群名，
      照抄）。**上游加族群時這裡要跟著加**，是刻意的顯式耦合。
    - `chg`（必填）／`pos`／`vr`／`t20` 是**數字**，照抄上游；上游給 null 就抄 null。
      程式驗型別與合理範圍（擋的是抄錯欄位、小數點抄掉，不是抄錯值）。
    - 版面上「昨夜」那一格**由 `render_body` 用這些數字排**，模型不經手。
    - `view` 是這區唯一的自由文字（4~24 字元），**不准夾帶數值**（`find_number`）。
      要擋的是「看起來像個數值」而不是「數字字元」，這兩者不一樣：
      阿拉伯／全形／羅馬／圈號／上下標／分數（Unicode Nd·Nl·No）無條件擋；
      中文數字要**接單位或接另一個中文數字**才算——「跌三趴」「近十日」擋，
      「半導體仍偏弱」「與大盤一致」放行。
      第一版訂成「擋數字字元」，2026-09-21 的 dry-run 當場踩到誤擋
      （半、一、萬 都在中文數字表裡，而「半導體」正是族群判讀會寫的詞）。
      誤擋一次就是整份晨報 exit 1，不是只砍那一列。
    - 至多 **6 列**；`us_sectors_date` 是**未來日期**也會擋。
    - `us_sectors_date` 必填、格式嚴格比對 `YYYY-MM-DD`、距今 >5 天擋，
      並渲染進區塊標題讓讀者知道「昨夜」是哪一夜。**不做美股假日表是刻意的**——
      要擋的是「來源停更卻沒人發現」。5 天＝美股連假最長 4 天再加一天餘裕。
    **為什麼不是規則而是改結構**：這條改過四版，前三版都是拿樣式規則管自由文字，
    三輪驗收三次被繞過，而且每次都是同一個原因——**寫的人永遠比過濾器有更多表達空間**：
    ｜v1 禁字表｜「熄火」擋掉就寫「退燒」「人氣散去」「籌碼鬆動」｜7 句漏 6｜
    ｜v2 漢字上限｜一句判決只要 4~6 漢字，真實寫法本身就用掉 6，長度分不開｜30 句漏 27｜
    ｜v3 白名單｜擋得住漢字，擋不住英文／注音／emoji／相容漢字；`g` 欄還留著 v1 那套｜15 句全過｜
    第四版把**問題本身**拿掉：數字不讓模型用文字重打，`g` 限定固定名單，
    只剩 `view` 是自由文字——而 view 本來就該是判讀。全形半形、分隔符號、
    字數失控、標籤重複這些問題一併消失。
    **但要誠實說清楚這一版擋得住什麼**：`view` 仍然寫得出「動能熄火」，
    驗證分不出那是在講昨夜還是多週——擋它只會把判讀欄變成不能寫判讀。
    真正在防的是**版面**：`chg`／`pos`／`vr` 與 `t20` 就排在 `view` 旁邊，
    寫錯會當場自相矛盾，而且那些數字不是它寫的。
    這是「讓錯誤顯眼」，不是「讓錯誤不可能」。驗證只保證一件事：
    view 裡不會出現**它自己編的數字**。測試把這個分界釘死了
    （帶數字的擋、純語氣的放行，兩邊都有斷言）。
    由來：2026-09-17 光通訊核心 −0.08%／收在區間 10% 被寫成「熄火」，
    9/18 就是 +3.96%／收在區間 58%（兩組數字都能用 `build_us_sectors.py` 的
    同一套算法重算驗證）。**一天的數字撐不起趨勢敘述。**
    `daily-brief-card.json` **不帶** `us_sectors`：LINE 長圖空間有限，族群表放不下。

## 跨 repo 依賴

- 讀 taiwan-flow-live-v2 的 `morning.json` / `us.json` / `daysummary/latest.json`
  （讀不到時該整段隱藏）。另有 `us_sector_flow.json`——**本 repo 沒有任何程式讀它**，
  是雲端排程 session 用 WebFetch 取來填 `us_sectors` 的；`brief_tools.py` 只驗結果。
