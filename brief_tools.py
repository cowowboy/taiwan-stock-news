...
# view 裡的數字偵測。要擋的不是「數字字元」,是「**看起來像個數值**」——
# 這兩者不一樣,而第一版訂成前者就誤擋了「半導體仍偏弱」「與大盤一致」
# 「萬全之策」（2026-09-21 dry-run 實際踩到;半、一、萬 都在中文數字表裡）。
#
# 規則分兩段:
#   阿拉伯／全形／羅馬／圈號／上下標／分數（Unicode Nd·Nl·No）→ 無條件擋。
#       這些在一句位置判讀裡沒有正當用途,出現就是在夾帶數值。
#   中文數字 → 後面要接單位、或接另一個中文數字,才算數值。
#       「跌三趴」「近十日」「回落兩成」「跌二十趴」擋;
#       「半導體」「一致」「一路走低」「萬全」放行。
CJK_NUM = "〇一二三四五六七八九十百千萬億兩半參壹貳叁肆伍陸柒捌玖拾佰仟"
# 接在中文數字後面會把它變成一個數值的字。百千萬億同時是數字也是單位。
CJK_UNIT = "趴成倍點日週月年元分檔支席百千萬億"
...
def find_number(s: str) -> str:
    """回傳第一個「像數值」的片段,沒有就回空字串。

    引整串而不是單一字元:「近20日」報「出現數字 2」讀起來像沒看懂,
    報「出現數字 20」才指得到人看得出來的東西。
    """
    for i, ch in enumerate(s):
        hit = unicodedata.category(ch) in ("Nd", "Nl", "No")
        if not hit and ch in CJK_NUM:
            nxt = s[i + 1] if i + 1 < len(s) else ""
            hit = nxt in CJK_NUM or nxt in CJK_UNIT
        if hit:
            j = i
            while j < len(s) and _is_num(s[j]):
                j += 1
            # 把單位一起引出來,「三趴」比「三」看得懂
            if j < len(s) and s[j] in CJK_UNIT:
                j += 1
            return s[i:j]
    return ""
  ...
  "us_sectors_date": "來源檔的資料日 YYYY-MM-DD"（us_sectors 非空時必填）
                 ...
                 **版面上的「昨夜」那一格是程式用這些數字排的，你不用寫。**
                 ...
                 view 是這區唯一的自由文字，寫一句位置判讀，如「仍在修正段」。
                   ...
                   **不准夾帶數值**：阿拉伯／全形數字、羅馬數字、圈號①、
                   上下標、分數½ 一律不准；中文數字則是「接單位」才擋——
                   「跌三趴」「近十日」「回落兩成」不行，
                   「半導體仍偏弱」「與大盤一致」「一路走低」可以。
